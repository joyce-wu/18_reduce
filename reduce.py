...
def word_freq(word, f):
    lst = [1 for w in f if w == word] #creates list of 1's for when word is found
    if lst == []:
        return 0
    return reduce((lambda x, y: x + y), lst) #adds up frequency of word using reduce
